...
#include <math.h>
#include <time.h>
#include <sys/sysinfo.h> // number of cores
...
boolean ApplyColorInversionOnPartialBuffer(unsigned char *data, unsigned long long size, int channels)
{
    ...
    size_t i = 0;
    #pragma omp parallel for schedule(static) shared(data) private(i)
...
void CreateGaussianKernel(struct GaussianKernel *kernel, unsigned char radius)
{
    ...
    int i = 0;
    int j = 0;
    // #pragma omp parallel shared(kernel) private(i, j)
	// {
    //     #pragma omp parallel for schedule(runtime) collapse(2) reduction(+: sum)
    for (i = 0; i < size; i++) 
    {
        for (j = 0; j < size; j++) 
        {
    ...
    struct GaussianKernel* kernel)
{
    ...
    #pragma omp parallel for schedule(static) collapse(2) firstprivate(size, width, height, channels, kernel) shared(data)
    for (i = 0; i < width; i++) 
    {
        for (j = 0; j < height; j++) 
        {
            ...
            // #pragma omp parallel for schedule(static) collapse(2) reduction(+: b, g, r) private(n, m)
            for (m = -kernel->radius; m < kernel->radius; m++) 
            {
                // #pragma omp parallel for schedule(static) reduction(+: b, g, r)
                for (n = -kernel->radius; n < kernel->radius; n++) 
                {
...
void FlipMatrix(unsigned char *data, unsigned long long width, unsigned long long height, int channels)
{
    ...
    unsigned int row = 0;
    unsigned int i = 0;
    #pragma omp parallel for schedule(static) shared(data) private(row, i) collapse(2)
...
{
    const int levels = omp_get_max_active_levels();
    ...
    const int coresNo = get_nprocs();
    omp_set_num_threads(coresNo);// / mContext.world.size);
    ...
    for (unsigned int i = 0; i < mContext.cmd.actionsNumber; i++)
    {
        ...
        const double time_taken = ((end.tv_sec - now.tv_sec) * 1000000u + end.tv_usec - now.tv_usec) / 1.e6;
